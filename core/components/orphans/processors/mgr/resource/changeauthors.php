...
if (!empty($scriptProperties['createdby'])) {
    ...
    if (empty($createdby)) return $modx->error->failure($modx->lexicon('orphans.user_err_nf'));
}
if (!empty($scriptProperties['editedby'])) {
    $editedby = $modx->getObject('modUser',$scriptProperties['createdby']);
    if (empty($editedby)) return $modx->error->failure($modx->lexicon('orphans.user_err_nf'));
}
if (!empty($scriptProperties['publishedby'])) {
    $publishedby= $modx->getObject('modUser',$scriptProperties['publishedby']);
    if (empty($publishedby)) return $modx->error->failure($modx->lexicon('orphans.user_err_nf'));
}
